...
def get_note_list(track, ticks_per_beat, bias_beat):
    """
    获取一首歌的一个一个音轨的音符列表。音符列表为二维数组 第一维为音符 第二维为音符属性（时间+音高+音量+持续时间）
    :param track: 一首歌的音轨
    :param ticks_per_beat: 每拍多少个ticks
    :param bias_beat: 偏移时间（即对整个歌曲进行平移，将重音移到每小节的第一拍）
    :return: np.array 音符列表
    """
    curr_beat = 0  # 当前拍子
    note_list = np.array([])  # 音符列表
    # 1.保存这个音轨中的所有音符
    for msg in track:
        curr_beat += msg.time / ticks_per_beat
        if msg.type == 'note_on':
...
def get_marked_note_list(tracks, ticks_per_beat, bias_beats, mark_name_dic):
    """
    按照音轨的功能将音符列表进行分类
    ...
    for track in tracks:
        ...
        for key in mark_name_dic:
            if key in track_name:  # 这个音轨里有mark_name_dict中的某个关键字
                ...
                note_list = get_note_list(track, ticks_per_beat, bias_beats)
                if len(note_list) != 0:
...
def get_multi_note_lists(tracks, ticks_per_beat, bias_time, mark_name_dic):
    """
    按照音轨的功能将音符列表分类。
    ...
    for track in tracks:
        ...
        for key in mark_name_dic:
            if key in track_name:  # 这个音轨里有mark_name_dict中的某个关键字
                ...
                note_list = get_note_list(track, ticks_per_beat, bias_time)
                if len(note_list) != 0:
                    marked_note_list[mark + mark_number].append(note_list)
    return marked_note_list


def generate_pianoroll(note_list):
    """
...
def generate_multi_pianoroll(marked_note_list):
    """
    ...
    for key in marked_note_list:
        if bool(marked_note_list[key]):
            pianoroll_dic[key] = generate_pianoroll(note_list=marked_note_list[key])
...
def adjust_pitch_to_average(pianoroll_dic):
    """
    ...
    for key in pianoroll_dic:
        ...
        for note in pianoroll_dic[key]:
            sum_pitch += note[1]
        avr_pitch = sum_pitch / total_note  # 音符的平均音高
        if key.startswith('piano_guitar'):
            pitch_diff = avr_pitch - PG_AVR_NOTE  # 音符的平均音高与平均音高的期望值相差多少
        elif key.startswith('string'):
            pitch_diff = avr_pitch - STRING_AVR_NOTE
        elif key.startswith('fill'):
            pitch_diff = avr_pitch - FILL_AVR_NOTE
        else:
            continue
        adjust_scale = 12 * round(pitch_diff / 12)  # 要调整的数量
        for note_it in range(len(pianoroll_dic[key])):
            pianoroll_dic[key][note_it][1] -= adjust_scale
    return pianoroll_dic


def generate_data_from_midi_file(file_name, bias_beat=0, scale=0):
    marked_track_list = dict()
    channel_list = []  # 音轨编号 在note_list中的位置
    tracks, ticks_per_beat = readfile(file_name)  # 1.获取一首歌的各个音轨
    marked_note_list = get_marked_note_list(tracks, ticks_per_beat, bias_beat,
                                            {'Main': 'main', 'Chord': 'chord', 'Drum': 'drum', 'Bass': 'bass', 'Interlude': 'interlude', 'Intro': 'intro'})  # 2.获取这首歌的音符列表 分音轨保存
    multi_tracks_note_list = get_multi_note_lists(tracks, ticks_per_beat, bias_beat,
                                                  {'Cl': 'string', 'Cs': 'piano_guitar', 'Fill': 'fill'})  # Cl是音符间隔时间较长的和弦表达 通常是有string来表现;Cs是音符间隔时间较短的和弦表达 通常是由piano和guitar来表现
    for key in multi_tracks_note_list:  # 把两个marked_note_list合并为一个
        marked_note_list[key] = multi_tracks_note_list[key]
    pianoroll_list = generate_multi_pianoroll(marked_note_list)  # 音符列表转成piano_roll
    pianoroll_list = adjust_scale(pianoroll_list, scale)  # 调整音高 （包含调整调式和平均音高)
    return pianoroll_list
