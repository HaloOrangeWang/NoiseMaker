...
from datainputs.drum import DrumTrainData, DrumTestData
...
class DrumPipeline(BaseLstmPipeline):
    ...
    def __init__(self, is_train, *args):
        if is_train:
            # 训练时的情况
            # args = (mmelody_pattern_data, continuous_bar_data)
            self.train_data = DrumTrainData(*args)
        else:
            self.train_data = DrumTestData()
        super().__init__()
    ...
    def generate(self, session, melody_output, melody_output_pattern, drum_test_input=None):
        ...
        while beat_dx <= len(melody_output_pattern):  # 遍历整个主旋律 步长是2拍
            ...
            if generate_fail_time > MAX_GEN_DRUM_FAIL_TIME:
            ...
            if melody_output_pattern[(beat_dx - 2): beat_dx] == [0, 0]:  # 最近这2拍没有主旋律 则鼓机沿用之前的
                ...
                drum_out = music_pattern_prediction(drum_predict, 1, COMMON_DRUM_PAT_NUM)  # 将二维数组predict通过概率随机生成这两拍的鼓点组合drum_out
