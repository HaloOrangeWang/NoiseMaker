...
class MelodyPipeline(BaseLstmPipeline):
    ...
    def __init__(self, tone_restrict):
    ...
    def prepare(self):
        ...
        self.train_data = MelodyTrainData(self.kmeans_sess, self.kmeans_model, self.cluster_center_points, tone_restrict=self.tone_restrict)
    ...
    def generate(self, session, melody_generate_input=None, begin_empty_bar_number=0):
        ...
        while True:
            ...
            if no_input_melody and iterator == 0:
                ...
                start_note_cluster = GetMelodyProfileBySong(self.kmeans_sess, self.kmeans_model, self.cluster_center_points, {0: melody_output[-8:] * 4, 1: melody_output[-8:] * 4})[1]  # 初始音符所在的cluster
            ...
            if iterator % 8 == 0:
                ...
                if not MelodyClusterCheck(self.kmeans_sess, self.kmeans_model, self.cluster_center_points, melody_output[-64:], melody_profile[int(iterator / 4) + initial_melody_profile_length - 1], 3, train_pattern=True):  # 检查这两小节的音乐cluster是否符合要求 如果不符合要求 则返工 重新生成这两1小节的音乐
