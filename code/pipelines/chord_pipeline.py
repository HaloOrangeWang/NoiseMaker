...
class ChordPipeline(BaseLstmPipeline):
    ...
    def generate(self, session, melody_output, common_melody_patterns):
        ...
        while melody_iterator <= len(melody_output_pattern):  # 遍历整个主旋律 步长是2拍
            ...
            # 2.3.检查生成的和弦
            if melody_iterator >= 10 and melody_iterator % 4 == 2 and generate_fail_time < MAX_CHORD_GENERATE_FAIL_TIME:
                if not ChordCheck(chord_output[-8:], melody_output[(melody_iterator - 10) * 8: (melody_iterator - 2) * 8]):  # 离调和弦的比例过高
                    print(melody_iterator, '1False%02d' % generate_fail_time, chord_output)
                    melody_iterator -= 8
                    chord_output = chord_output[:(-8)]
                    generate_fail_time += 1
                    continue
            if melody_iterator >= 14 and generate_fail_time < MAX_CHORD_GENERATE_FAIL_TIME:
                if len(set(chord_output[-12:])) == 1:  # 连续三小节和弦不变化
                    print(melody_iterator, '2False%02d' % generate_fail_time, chord_output)
                    melody_iterator -= 12
                    chord_output = chord_output[:(-12)]
                    generate_fail_time += 1
                    continue
            if melody_iterator > len(melody_output_pattern) and generate_fail_time < MAX_CHORD_GENERATE_FAIL_TIME:
                if chord_output[-1] != 1:  # 最后一个和弦不是1级大和弦
                    print(melody_iterator, '3False%02d' % generate_fail_time, chord_output)
                    melody_iterator -= 8
                    chord_output = chord_output[:(-8)]
                    generate_fail_time += 1
                    continue
