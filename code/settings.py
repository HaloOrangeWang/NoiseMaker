...
GENERATE_MIDIFILE_PATH = '../test.mid'  # 生成的midi文件所在路径
...
COMMON_MELODY_PATTERN_NUMBER = 250  # 常见的多少种主旋律的组合
...
FLAG_RUN_TEST_CASE = False  # 为True运行测试用例
...
MAX_MELODY_GENERATE_FAIL_TIME = 20  # 最大可接受的的主旋律生成的不合要求的次数。超过此次数则打回从头生成
MAX_CHORD_GENERATE_FAIL_TIME = 50  # 最大可接受的的和弦生成的不合要求的次数。超过此次数则打回从头生成
MAX_DRUM_GENERATE_FAIL_TIME = 10  # 最大可接受的的打击乐生成的不合要求的次数。超过此次数则打回从头生成

CHORD_DICT = [  # 和弦组合的词典 第1-6列分别是 大三 小三 增三 减三 挂二 挂四
    {-1},  # 把0号位空出来 用作未知和弦
