...
def logger(diary_id):
    ...
    if diary_id == -1:
        diary_id = 1
        while os.path.exists(DIARY_PATH % (status, curdate.year % 100, curdate.month, curdate.day, diary_id)):
            diary_id += 1
    diary_path = DIARY_PATH % (status, curdate.year % 100, curdate.month, curdate.day, diary_id)
    # 2.定义一个logger
