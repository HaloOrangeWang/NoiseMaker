...
def notelist2chord(note_set, saved_chord=0, tone=TONE_MAJOR, accompany_note_set=None):
    ...
    if len(new_note_set) == 2:
        ...
        for chord_it in range(len(recommand_chord_list)):
            if new_note_set.issubset(recommand_chord_list[chord_it]):
                return CHORD_DICT.index(recommand_chord_list[chord_it])
    ...
    if len(new_note_set) >= 4:
        ...
        for chord_it in range(len(recommand_chord_list)):
            if recommand_chord_list[chord_it].issubset(new_note_set):
                return CHORD_DICT.index(recommand_chord_list[chord_it])
        # 5.4.如果和弦字典中有和弦是它的子集 那么返回它在列表中的位置
        for chord_it in range(1, 73):  # 1-72号位是三和弦
            if CHORD_DICT[chord_it].issubset(new_note_set):
                return chord_it
        # 5.5.其他情况 返回未知和弦
        return 0
...
def note_list_to_7chord(note_set):
    ...
    if len(note_set) == 3:  # 有三个音的情况
        for chord_it in range(73, 109):
            if note_set.issubset(CHORD_DICT[chord_it]):  # 在CHORD_DICT中，73-108是七和弦
                return chord_it
        return 0
    elif len(note_set) >= 4:  # 大于等于四个音的情况
        for chord_it in range(73, 109):
            if CHORD_DICT[chord_it].issubset(note_set):
                return chord_it
        return 0
    else:
        return 0
...
def chord_to_3(chord_dx):
    """
    将一个和弦转化为三和弦
    :param chord_dx: 和弦在CHORD_DICT中的位置
    :return: 转化之后的三和弦在CHORD_DICT中的位置
    """
    if chord_dx <= 72 and chord_dx % 6 in range(1, 5):  # 大三/小三/增三/减三
        return chord_dx
    if chord_dx <= 72 and chord_dx % 6 in [0, 5]:  # 挂二/挂四
        return -1
    if chord_dx >= 72:  # 七和弦
        if chord_dx % 3 in [1, 0]:
            return ((chord_dx - 73) // 3) * 6 + 1
        else:
            return ((chord_dx - 73) // 3) * 6 + 2


def chord_rootnote(chord_dx, last_root, expect_root_base):
    """
    ...
    root_high_diff = chord_root - expect_root
